  ...
  corridors:[
    {id:"c41",sectorId:"sec1",number:41,bays:[
      {id:"c41b1e",number:1,side:"Esquerdo",label:"Tintas Látex",floors:[
        {id:"f3a",number:3,boxes:[{id:"bx1",sku:"1026780",qty:8,updatedBy:"Caio",date:"05/05/2025",validade:""}]},
        {id:"f2a",number:2,boxes:[{id:"bx2",sku:"1024276",qty:10,updatedBy:"Caio",date:"11/05/2025",validade:""}]},
    ...
    {id:"c44",sectorId:"sec2",number:44,bays:[
      {id:"c44b1e",number:1,side:"Esquerdo",label:"Bases e Registros",floors:[
        ...
        {id:"f2d",number:2,boxes:[{id:"bx3",sku:"1024256",qty:20,updatedBy:"Caio",date:"11/03/2025",validade:""}]},
        {id:"f1e",number:1,boxes:[{id:"bx4",sku:"1024274",qty:6,updatedBy:"Caio",date:"22/03/2025",validade:""},{id:"bx5",sku:"1798178",qty:58,updatedBy:"Caio",date:"22/03/2025",validade:""}]}
      ]},
...
async function persist(d){try{await window.storage.set("aereo-v5",JSON.stringify(d));}catch(_){}}
async function loadPersisted(){try{const r=await window.storage.get("aereo-v5");return r?JSON.parse(r.value):null;}catch(_){return null;}}
...
// ─── BOX CARD ─────────────────────────────────────────────────────────────────
function BoxCard({box,mascot,product,onDragStart,onDragEnd,onClick,isDragging,highlight}){
  const vi=getValidity(box.validade);
  const bdrColor=highlight?"#1dd1a1":vi&&vi.days<=90?vi.color+"88":"rgba(29,209,161,0.3)";
  return (
    <div draggable onDragStart={onDragStart} onDragEnd={onDragEnd} onClick={onClick} className="fin"
      style={{position:"relative",width:115,flexShrink:0,background:highlight?"rgba(29,209,161,0.12)":"rgba(10,50,68,0.9)",
        ...
        opacity:isDragging?0.25:1,transition:"opacity .15s",boxShadow:highlight?"0 0 14px rgba(29,209,161,0.35)":"0 2px 10px rgba(0,0,0,0.45)"}}>
  ...
  return (
    <Modal onClose={onClose} title={`📦 SKU ${box.sku}`}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
        {[
          {icon:"📦",label:"Quantidade",val:String(box.qty),color:null},
          {icon:"📍",label:"Local",val:`C${corridor.number} B${bay.number} A${floorNumber}`,color:null},
          {icon:"📅",label:"Validade",val:box.validade||"—",color:vi?.color},
        ].map((s,i)=>(
...
function BoxEditModal({modal,products,onSave,onClose,onDelete}){
  ...
  const [form,setForm]=useState(isEdit?{...modal.box}:{sku:"",qty:"",updatedBy:"",date:todayFull(),validade:""});
  ...
  useEffect(()=>{
    if(valdMode==="calcular"&&fabDate.length===10&&meses){const v=calcVenc(fabDate,meses);if(v)set("validade",v);}
  },[fabDate,meses,valdMode]);
  function submit(){
  ...
  return (
    <Modal onClose={onClose} title={isEdit?"✏️ Editar Caixa":"📦 Nova Caixa"}>
      ...
      {form.sku&&!product&&<div style={{marginTop:6,fontSize:11,color:C.dim,background:"rgba(255,255,255,0.04)",borderRadius:8,padding:"6px 10px"}}>SKU não cadastrado — adicione em Produtos depois.</div>}
      <Gap/>
      <Row>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8}}>
        {["direta","calcular"].map(m=>(
          <button key={m} onClick={()=>setValdMode(m)}
            style={{flex:1,background:valdMode===m?C.accentDim:"rgba(255,255,255,0.05)",border:`1px solid ${valdMode===m?"rgba(29,209,161,0.4)":C.border}`,color:valdMode===m?C.accent:C.muted,borderRadius:8,padding:"7px",fontSize:12,fontWeight:600}}>
      ...
      {valdMode==="calcular"&&(
        <>
          <Row>
            <div><Lbl>Data fabricação</Lbl><DateInput value={fabDate} onChange={setFabDate}/></div>
            <div><Lbl>Meses validade</Lbl><NumInput value={meses} onChange={setMeses} placeholder="12"/></div>
          </Row>
...
function ProductsScreen({products,onBack,onSaveProduct,onDeleteProduct}){
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh"}}>
        ...
        {list.map(p=>(
          <div key={p.sku} onClick={()=>setModal({product:p})} className="ch fin"
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:6}}>
              <Tag>{p.sku}</Tag>
              <div style={{display:"flex",gap:4}}>{p.um&&<Tag bg="rgba(255,255,255,0.06)" color={C.muted}>{p.um}</Tag>}</div>
            </div>
      ...
      {modal&&<ProductModal product={modal.product} onSave={p=>{onSaveProduct(p);setModal(null);}} onClose={()=>setModal(null)} onDelete={()=>{onDeleteProduct(modal.product.sku);setModal(null);}}/>}
    </div>
  );
}
        ...
        {items.map((item,i)=>(
          <div key={i} className="ch fin" onClick={()=>onNavigate({corridorId:item.cor.id,bayId:item.bay.id,boxId:item.box.id})}
            ...
            <div style={{paddingLeft:8}}>
              ...
              <div style={{fontSize:10,color:item.v.color}}>📅 Validade: {item.box.validade} → Ir para o local ›</div>
            </div>
          </div>
        ))}
      </div>
...
function SearchOverlay({data,onClose,onNavigate}){
  ...
  const results = term.length>=2 ? (()=>{
    ...
    Object.values(products).forEach(p=>{
      if(p.desc?.toLowerCase().includes(term.toLowerCase())||p.familia?.toLowerCase().includes(term.toLowerCase())) matchedSkus.add(p.sku);
    });
  ...
  const suggestions = term.length>=2&&term.length<6 ? [
    ...allBoxSkus.filter(s=>s.startsWith(term)&&s!==term),
    ...Object.values(products).filter(p=>p.desc?.toLowerCase().startsWith(term.toLowerCase())&&!allBoxSkus.includes(p.sku)).map(p=>p.sku)
  ].slice(0,5) : [];
  ...
  return (
    <div style={{position:"fixed",inset:0,background:C.bg,zIndex:300,display:"flex",flexDirection:"column"}}>
      ...
      {suggestions.length>0&&(
        <div style={{padding:"8px 14px 0",borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",paddingBottom:8}}>
            {suggestions.map(s=>(
              <button key={s} onClick={()=>setQuery(s)}
                style={{background:C.accentDim,border:`1px solid rgba(29,209,161,0.2)`,color:C.accent,borderRadius:8,padding:"4px 10px",fontSize:12,fontWeight:600}}>{s}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
        {term.length>=2&&results.length===0&&(
          <div style={{textAlign:"center",padding:"40px 0",color:C.muted}}>
            ...
            <div>Nenhum resultado para "{term}"</div>
          </div>
        )}
        {term.length<2&&(
          <div style={{textAlign:"center",padding:"40px 0",color:C.dim}}>
            <div style={{fontSize:28,marginBottom:8}}>💡</div>
            <div style={{fontSize:13}}>Digite o SKU ou nome do produto</div>
          </div>
        )}
        {results.map((r,i)=>(
          <div key={i} className="fin" style={{background:"rgba(255,255,255,0.05)",border:`1px solid ${C.border}`,borderRadius:12,padding:"12px",marginBottom:10}}>
            ...
            {r.locations.map((l,j)=>(
              <button key={j} onClick={()=>{onNavigate({corridorId:l.cor.id,bayId:l.bay.id,boxId:l.box.id});onClose();}}
                className="ch"
...
// ─── BAY SCREEN ──────────────────────────────────────────────────────────────
function BayScreen({bay,corridor,products,corridors,onBack,onUpdateBay,highlightBoxId}){
  const [modal,setModal]=useState(null);
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",position:"relative"}}>
      ...
      <div style={{padding:"16px 16px 12px",display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,background:C.bg,zIndex:10,borderBottom:`1px solid ${C.border}`}}>
        ...
        <Tag>{floors.length} and.</Tag><Tag>{totalBoxes} cx.</Tag>
      </div>
      <div style={{padding:"14px 14px 80px"}}>
        {floors.map(floor=>(
          <div key={floor.id} className={dragOverFloor===floor.id?"floor-drop":""}
            onDragOver={e=>{e.preventDefault();setDragOverFloor(floor.id);}}
            onDragLeave={()=>setDragOverFloor(null)} onDrop={()=>handleDrop(floor.id)}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              ...
              <div style={{display:"flex",gap:6}}>
                ...
                <button onClick={()=>{if(floor.boxes.length>0){alert("Remova as caixas antes.");return;}updateFloors(bay.floors.filter(f=>f.id!==floor.id));}}
...
function CorridorModal({corridor,onSave,onClose}){
  ...
  return (
    <Modal onClose={onClose} title={corridor?"✏️ Editar Corredor":"➕ Novo Corredor"}>
      <Lbl req>Número do Corredor</Lbl>
      <NumInput value={String(number)} onChange={v=>setNumber(v)} placeholder="Ex: 41"/>
      <SaveBtn onClick={()=>{if(!number){alert("Número obrigatório!");return;}onSave(Number(number));}}/>
    </Modal>
  );
}
  ...
  return (
    <Modal onClose={onClose} title={sector?"✏️ Editar Setor":"➕ Novo Setor"}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8}}>
        {emojis.map(e=>(
          <button key={e} onClick={()=>setMascot(e)}
            style={{background:mascot===e?C.accentDim:"rgba(255,255,255,0.06)",border:`1px solid ${mascot===e?"rgba(29,209,161,0.4)":C.border}`,borderRadius:8,padding:"6px 10px",fontSize:20}}>
            {e}
          </button>
        ))}
      </div>
...
// ─── CORRIDOR SCREEN ──────────────────────────────────────────────────────────
function CorridorScreen({corridor,products,corridors,onBack,onUpdateCorridor,highlightBayId,highlightBoxId}){
  const [selectedBay,setSelectedBay]=useState(highlightBayId||null);
  const [bayModal,setBayModal]=useState(null);

  const sector = null; // mascot comes from corridor itself via parent
  if(selectedBay){
    ...
    return <BayScreen bay={bay} corridor={corridor} products={products} corridors={corridors}
      highlightBoxId={highlightBoxId} onBack={()=>setSelectedBay(null)}
      ...
      <div className="fin" style={{background:"rgba(255,255,255,0.06)",border:`1px solid ${C.border}`,borderRadius:12,marginBottom:8,overflow:"hidden",position:"relative"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px 6px",borderBottom:`1px solid rgba(255,255,255,0.06)`}}>
          ...
          <div style={{display:"flex",gap:4}}>
            ...
            <button onClick={e=>{e.stopPropagation();deleteBay(bay.id);}} style={{background:"none",border:"1px solid rgba(255,107,107,0.2)",color:C.danger,borderRadius:6,padding:"3px 8px",fontSize:11}}>🗑</button>
          </div>
        </div>
...
// ─── SECTOR SCREEN ────────────────────────────────────────────────────────────
function SectorScreen({sector,corridors,products,allCorridors,onBack,onUpdateCorridor,onAddCorridor,onDeleteCorridor,highlightCorridorId,highlightBayId,highlightBoxId}){
  ...
  if(selectedCorridorId){
    ...
    return <CorridorScreen corridor={cor} products={products} corridors={allCorridors} onBack={()=>setSelectedCorridorId(null)}
      highlightBayId={highlightBayId} highlightBoxId={highlightBoxId}
      onUpdateCorridor={updated=>onUpdateCorridor(updated)}/>;
  }

  function handleCorridorSave(number){
    if(corridorModal.corridor){
      onUpdateCorridor({...corridorModal.corridor,number});
    } else {
      onAddCorridor({id:genId(),sectorId:sector.id,number,mascot:sector.mascot,bays:[]});
    }
    setCorridorModal(null);
  }
  function deleteCorridor(corId){
    const cor=corridors.find(c=>c.id===corId);
    const total=cor.bays.reduce((s,b)=>s+b.floors.reduce((s2,f)=>s2+f.boxes.length,0),0);
    if(total>0){alert("Remova todas as caixas antes de excluir o corredor.");return;}
    onDeleteCorridor(corId);
  }
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",position:"relative"}}>
      ...
      <div style={{padding:"16px 16px 12px",display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,background:C.bg,zIndex:10,borderBottom:`1px solid ${C.border}`}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:C.muted,fontSize:22,padding:"0 4px 0 0"}}>←</button>
        <div style={{flex:1}}>
          <div style={{fontWeight:700,fontSize:17}}>{sector.mascot} {sector.name}</div>
          <div style={{fontSize:11,color:C.muted}}>{corridors.length} corredor(es)</div>
        </div>
      ...
      <div style={{padding:14}}>
        {corridors.length===0&&(
          <div style={{textAlign:"center",padding:"40px 0",color:C.muted}}>
            <div style={{fontSize:40,marginBottom:12}}>{sector.mascot}</div>
            <div style={{fontSize:13}}>Nenhum corredor neste setor.<br/>Toque em + Corredor para adicionar.</div>
          </div>
        )}
            ...
            <div key={cor.id} className="fin" style={{background:"rgba(255,255,255,0.06)",border:`1px solid ${C.border}`,borderRadius:12,marginBottom:8,overflow:"hidden",position:"relative"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px 6px",borderBottom:`1px solid rgba(255,255,255,0.06)`}}>
                ...
                <div style={{display:"flex",gap:4}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();deleteCorridor(cor.id);}} style={{background:"none",border:"1px solid rgba(255,107,107,0.2)",color:C.danger,borderRadius:6,padding:"3px 8px",fontSize:11}}>🗑</button>
                </div>
              </div>
...
// ─── HOME SCREEN ──────────────────────────────────────────────────────────────
function HomeScreen({data,onSelectSector,onOpenProducts,onOpenValidity,onOpenSearch,onEditSector,onDeleteSector,onAddSector}){
    ...
    <div style={{background:C.bg,minHeight:"100vh",position:"relative"}}>
      ...
      <div style={{padding:"22px 16px 14px",borderBottom:`1px solid ${C.border}`}}>
        ...
        <button onClick={onOpenSearch} style={{background:"rgba(255,255,255,0.08)",border:`1px solid ${C.border}`,borderRadius:10,padding:"11px 14px",color:C.dim,fontSize:14,width:"100%",textAlign:"left",display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:15}}>🔍</span> Buscar por SKU ou nome do produto...
        </button>
      </div>
            ...
            return (
              <div key={sector.id} className="fin" style={{background:"rgba(255,255,255,0.06)",border:`1px solid ${C.border}`,borderRadius:14,overflow:"hidden",position:"relative"}}>
                <div style={{display:"flex",justifyContent:"flex-end",padding:"6px 8px 0",gap:4,position:"relative",zIndex:1}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();onDeleteSector(sector.id);}} style={{background:"none",border:"1px solid rgba(255,107,107,0.2)",color:C.danger,borderRadius:6,padding:"2px 6px",fontSize:10}}>🗑</button>
                </div>
...
export default function App(){
  ...
  function deleteSector(id){
    if((data.corridors||[]).some(c=>c.sectorId===id)){alert("Remova todos os corredores deste setor antes de excluí-lo.");return;}
  ...
  return (
    <>
      ...
      {screen.type==="sector"&&(()=>{
        ...
        return <SectorScreen sector={sector} corridors={corridors} products={data.products} allCorridors={data.corridors.map(c=>({...c,mascot:(data.sectors||[]).find(s=>s.id===c.sectorId)?.mascot||"📦"}))}
          onBack={back} onUpdateCorridor={updateCorridor} onAddCorridor={addCorridor} onDeleteCorridor={deleteCorridor}/>;
      })()}
      {screen.type==="bay"&&(()=>{
        ...
        return <BayScreen bay={bay} corridor={corridor} products={data.products}
          corridors={data.corridors.map(c=>({...c,mascot:(data.sectors||[]).find(s=>s.id===c.sectorId)?.mascot||"📦"}))}
          highlightBoxId={screen.highlightBoxId} onBack={back}
          onUpdateBay={updated=>updateCorridor({...cor,bays:cor.bays.map(b=>b.id===updated.id?updated:b)})}/>;
      })()}
      {screen.type==="products"&&<ProductsScreen products={data.products} onBack={back} onSaveProduct={saveProduct} onDeleteProduct={deleteProduct}/>}
      {screen.type==="validity"&&<ValidityScreen data={data} onBack={back} onNavigate={v=>{handleNavigate(v);}}/>}
