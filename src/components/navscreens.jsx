...
export function BayScreen({bay,corridor,products,corridors,onBack,onUpdateBay,onUpdateBayStructure,highlightBoxId,onConfirmDelete,onRegisterUndo,profile}){
  ...
  function doDeleteBox(){
    ...
    onRegisterUndo("Caixa excluída", ()=>db.deleteBox(boxId));
    setModal(null);setDetailModal(null);
  }
      ...
      floors.map(floor=>React.createElement("div",{key:floor.id,style:{background:"rgba(0,0,0,0.25)",border:"1px solid "+C.border,borderRadius:12,padding:"10px 10px 4px",marginBottom:10}},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}},
          ...
          React.createElement("div",{style:{display:"flex",gap:6}},
            isOperator(profile)&&React.createElement("button",{onClick:()=>setModal({type:"add",floorId:floor.id}),style:{background:C.accentDim,border:"1px solid rgba(29,209,161,0.25)",color:C.accent,borderRadius:7,padding:"2px 11px",fontSize:19,lineHeight:"1.2"}},"+"),
    ...
    return React.createElement("div",{className:"fin",style:{background:"rgba(255,255,255,0.06)",border:"1px solid "+C.border,borderRadius:12,marginBottom:8,overflow:"hidden",position:"relative"}},
      ...
      React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px 6px",borderBottom:"1px solid rgba(255,255,255,0.06)"}},
        ...
        React.createElement("div",{style:{display:"flex",gap:4}},
          isEndministrator(profile)&&React.createElement("button",{onClick:e=>{e.stopPropagation();setBayModal({side:bay.side,bay});},style:{background:"none",border:"1px solid "+C.border,color:C.muted,borderRadius:6,padding:"3px 8px",fontSize:11}},"✏️"),
          isEndministrator(profile)&&React.createElement("button",{onClick:e=>{e.stopPropagation();onConfirmDelete("Excluir Bay "+bay.number+"?",()=>{
            if(bay.floors.reduce((s,f)=>s+f.boxes.length,0)>0){alert("Remova todas as caixas antes.");return;}
            onRegisterUndo("Bay "+bay.number+" excluído", ()=>db.deleteBay(bay.id));
  ...
  return React.createElement("div",{style:{background:C.bg,minHeight:"100vh",position:"relative"}},
    ...
    React.createElement("div",{style:{padding:14}},
      ...
      React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}},
        React.createElement("div",null,leftBays.map(b=>React.createElement(BayCard,{key:b.id,bay:b})),isEndministrator(profile)&&React.createElement("button",{onClick:()=>setBayModal({side:"Esquerdo",bay:null}),style:{background:"none",border:"1px dashed "+C.border,color:C.muted,borderRadius:10,padding:9,width:"100%",fontSize:12,marginBottom:8}},"+ Bay Esq.")),
        React.createElement("div",null,rightBays.map(b=>React.createElement(BayCard,{key:b.id,bay:b})),isEndministrator(profile)&&React.createElement("button",{onClick:()=>setBayModal({side:"Direito",bay:null}),style:{background:"none",border:"1px dashed "+C.border,color:C.muted,borderRadius:10,padding:9,width:"100%",fontSize:12,marginBottom:8}},"+ Bay Dir."))
      )
    ),
      ...
      [...corridors].sort((a,b)=>a.number-b.number).map(cor=>{
        ...
        return React.createElement("div",{key:cor.id,className:"fin",style:{background:"rgba(255,255,255,0.06)",border:"1px solid "+C.border,borderRadius:12,marginBottom:8,overflow:"hidden",position:"relative"}},
          ...
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px 6px",borderBottom:"1px solid rgba(255,255,255,0.06)"}},
            ...
            React.createElement("div",{style:{display:"flex",gap:4}},
              isEndministrator(profile)&&React.createElement("button",{onClick:e=>{e.stopPropagation();setCorridorModal({corridor:cor});},style:{background:"none",border:"1px solid "+C.border,color:C.muted,borderRadius:6,padding:"3px 8px",fontSize:11}},"✏️"),
              isEndministrator(profile)&&React.createElement("button",{onClick:e=>{e.stopPropagation();onConfirmDelete("Excluir Corredor "+cor.number+"?",()=>{
                ...
                onRegisterUndo("Corredor "+cor.number+" excluído", ()=>db.deleteCorridor(cor.id));
  ...
  return React.createElement("div",{style:{background:C.bg,minHeight:"100vh",position:"relative"}},
    ...
    React.createElement("div",{style:{padding:"22px 16px 14px",borderBottom:"1px solid "+C.border}},
      ...
      React.createElement("div",{style:{display:"flex",gap:8,marginBottom:14}},
        React.createElement("button",{onClick:onOpenProducts,className:"nav-btn",style:{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid "+C.border,color:C.muted,borderRadius:10,padding:"10px 8px",fontWeight:700,fontSize:12}},"🗂 Produtos"),
        React.createElement("button",{onClick:onOpenValidity,className:hasExpiring?"blink nav-btn":"nav-btn",style:{flex:1,background:hasExpiring?"rgba(255,107,107,0.12)":"rgba(255,255,255,0.06)",border:"1px solid "+(hasExpiring?"rgba(255,107,107,0.3)":C.border),color:hasExpiring?C.danger:C.muted,borderRadius:10,padding:"10px 8px",fontWeight:700,fontSize:12}},hasExpiring?"⚠️ Validades ("+expiringItems.length+")":"⏰ Validades"),
        isEndministrator(profile)&&React.createElement("button",{onClick:onOpenOperators,className:"nav-btn",style:{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid "+C.border,color:C.muted,borderRadius:10,padding:"10px 8px",fontWeight:700,fontSize:12}},"👥 Operators")
      ),
    ...
    !term&&React.createElement("div",{style:{padding:"16px 16px 60px"}},
      React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}},
        ...
        isEndministrator(profile)&&React.createElement("button",{onClick:()=>setSectorModal({sector:null}),style:{background:"none",border:"1px solid "+C.border,color:C.muted,borderRadius:8,padding:"4px 10px",fontSize:11}},"+ Setor")
      ),
        ...
        (data.sectors||[]).map(sector=>{
          ...
          return React.createElement("div",{key:sector.id,className:"fin",style:{background:"rgba(255,255,255,0.06)",border:"1px solid "+C.border,borderRadius:14,overflow:"hidden",position:"relative"}},
            React.createElement("div",{style:{display:"flex",justifyContent:"flex-end",padding:"6px 8px 0",gap:4,position:"relative",zIndex:1}},
              isEndministrator(profile)&&React.createElement("button",{onClick:e=>{e.stopPropagation();setSectorModal({sector});},style:{background:"none",border:"1px solid "+C.border,color:C.muted,borderRadius:6,padding:"2px 6px",fontSize:10}},"✏️"),
              isEndministrator(profile)&&React.createElement("button",{onClick:e=>{e.stopPropagation();onConfirmDelete("Excluir setor \""+sector.name+"\"?",()=>{
                if((data.corridors||[]).some(c=>c.sectorId===sector.id)){alert("Remova todos os corredores deste setor antes.");return;}
                onRegisterUndo("Setor \""+sector.name+"\" excluído", ()=>db.deleteSector(sector.id));
